...
function getMockConfigs({ appPath, mocks, helper }) {
    ...
    if (helper.fs.existsSync(mockDir)) {
        ...
        if (mockFiles.length) {
            ...
            let createRegister;
            if ('createBabelRegister' in helper)
                createRegister = helper.createBabelRegister;
            if ('createSwcRegister' in helper)
